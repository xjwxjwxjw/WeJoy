  <!-- content start -->
  <script src={{url("/admin/js/friendlylink_app.js")}}></script>
<!-- <div id="slide-target"> -->
    ...
    <div class="am-g">
      <div class="am-u-sm-12">
          <table class="am-table am-table-striped am-table-hover table-main">
            <thead>
              <tr>
                <th class="table-check"><input type="checkbox" /></th><th class="table-id">ID</th><th class="table-title">name</th><th class="table-title">url</th><th class="table-date">发布日期</th><th class="table-date">修改日期</th><th class="table-set">操作</th>
              </tr>
          </thead>
          <tbody id="task-list">
            @if(empty($tasks))
            ...
            @foreach ($tasks as $v)
              <tr id="task{{ $v->id }}">
                ...
                <td>{{$v->id}}</td>
                <td>{{$v->name}}</td>
                <td><span class="my-con" title="{{$v->url}}">{{$v->url}}</span></td>
                <td>{{$v->created_at}}</td>
                <td>{{$v->updated_at}}</td>
                <td>
          ...
          <div class="am-cf">
             <nav aria-label="...">
               @if( !empty($keepsearch) )
                {{$tasks->appends(['search' => $keepsearch])->links('admin/friendlylink.page')}}
               @else
                {{$tasks->links('admin/friendlylink.page')}}
               @endif
            </nav>
          </div>
          <hr />
