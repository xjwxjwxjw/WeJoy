...
var ZYFILE = {
	...
	funUploadFile : function(file){
		...
		xhr.open("POST",self.url+"?_token="+token, true);
...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '					<div class="upload_btn">发布</div>';
		            html += '				</div>';
		            html += '			</div>';
...
//								}
			...
			this.addEvent = function(){
				...
				$(".upload_btn").bind("click", function(e){
					// 判断当前是否有文件需要上传
					if(ZYFILE.funReturnNeedFiles().length > 0 && ZYFILE.funReturnNeedFiles().length <= 9 && $("#textarea").val().length > 0 ){
						$("#fileSubmit").click();
					}else if( ZYFILE.funReturnNeedFiles().length > 9 ){
						alert("超出图片上限,最多9张");
